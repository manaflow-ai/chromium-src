...
BOOL FinishStatus(int status, char* c_error, NSError** error) {
  ...
  if (error) {
    NSString* message = c_error ? [NSString stringWithUTF8String:c_error]
                                : @"unknown Mojo runtime error";
...
static void OwlFreshMojoRuntimeEventThunk(
    const owl_fresh::OwlFreshMojoEvent* event,
    void* user_data) {
  if (!event || !user_data) {
    return;
  }
...
- (BOOL)setClientWithHandle:(uint64_t)handle error:(NSError**)error {
  ...
  return FinishStatus(
      owl_fresh::owl_fresh_mojo_session_set_client(_session, handle, &c_error),
      c_error, error);
}
...
- (BOOL)setClientWithRemoteHandle:(uint64_t)remoteHandle
                   receiverHandle:(uint64_t)receiverHandle
                            error:(NSError**)error {
  char* c_error = nullptr;
  return FinishStatus(owl_fresh::owl_fresh_mojo_session_set_client_with_handles(
                          _session, remoteHandle, receiverHandle, &c_error),
                      c_error, error);
}
...
- (BOOL)bindProfileWithRemoteHandle:(uint64_t)remoteHandle
  ...
  char* c_error = nullptr;
  return FinishStatus(
      owl_fresh::owl_fresh_mojo_session_bind_profile_with_handles(
          _session, remoteHandle, receiverHandle, &c_error),
      c_error, error);
}
...
- (BOOL)bindProfileWithReceiverHandle:(uint64_t)receiverHandle
                                error:(NSError**)error {
  char* c_error = nullptr;
  return FinishStatus(owl_fresh::owl_fresh_mojo_session_bind_profile_receiver(
...
- (BOOL)bindWebViewWithHandle:(uint64_t)handle error:(NSError**)error {
  char* c_error = nullptr;
  return FinishStatus(owl_fresh::owl_fresh_mojo_session_bind_web_view(
                          _session, handle, &c_error),
                      c_error, error);
}
...
- (BOOL)bindWebViewWithRemoteHandle:(uint64_t)remoteHandle
  ...
  char* c_error = nullptr;
  return FinishStatus(
      owl_fresh::owl_fresh_mojo_session_bind_web_view_with_handles(
          _session, remoteHandle, receiverHandle, &c_error),
      c_error, error);
}
...
- (BOOL)bindInputWithHandle:(uint64_t)handle error:(NSError**)error {
  char* c_error = nullptr;
  return FinishStatus(
      owl_fresh::owl_fresh_mojo_session_bind_input(_session, handle, &c_error),
      c_error, error);
}
...
- (BOOL)bindInputWithRemoteHandle:(uint64_t)remoteHandle
                   receiverHandle:(uint64_t)receiverHandle
                            error:(NSError**)error {
  char* c_error = nullptr;
  return FinishStatus(owl_fresh::owl_fresh_mojo_session_bind_input_with_handles(
                          _session, remoteHandle, receiverHandle, &c_error),
                      c_error, error);
}
...
- (BOOL)bindSurfaceTreeWithRemoteHandle:(uint64_t)remoteHandle
  ...
  char* c_error = nullptr;
  return FinishStatus(
      owl_fresh::owl_fresh_mojo_session_bind_surface_tree_with_handles(
          _session, remoteHandle, receiverHandle, &c_error),
      c_error, error);
}
...
- (BOOL)bindSurfaceTreeWithReceiverHandle:(uint64_t)receiverHandle
  ...
  char* c_error = nullptr;
  return FinishStatus(
      owl_fresh::owl_fresh_mojo_session_bind_surface_tree_receiver(
          _session, receiverHandle, &c_error),
      c_error, error);
}
...
- (BOOL)bindNativeSurfaceHostWithHandle:(uint64_t)handle
  ...
  char* c_error = nullptr;
  return FinishStatus(
      owl_fresh::owl_fresh_mojo_session_bind_native_surface_host(
          _session, handle, &c_error),
      c_error, error);
}
...
- (BOOL)bindNativeSurfaceHostWithRemoteHandle:(uint64_t)remoteHandle
  ...
  char* c_error = nullptr;
  return FinishStatus(
      owl_fresh::owl_fresh_mojo_session_bind_native_surface_host_with_handles(
          _session, remoteHandle, receiverHandle, &c_error),
      c_error, error);
}
...
- (BOOL)bindDevToolsHostWithRemoteHandle:(uint64_t)remoteHandle
  ...
  char* c_error = nullptr;
  return FinishStatus(
      owl_fresh::owl_fresh_mojo_session_bind_devtools_host_with_handles(
          _session, remoteHandle, receiverHandle, &c_error),
      c_error, error);
}
...
- (BOOL)bindDevToolsHostWithReceiverHandle:(uint64_t)receiverHandle
  ...
  char* c_error = nullptr;
  return FinishStatus(
      owl_fresh::owl_fresh_mojo_session_bind_devtools_host_receiver(
          _session, receiverHandle, &c_error),
      c_error, error);
}
...
- (nullable NSNumber*)flushWithError:(NSError**)error {
  ...
  char* c_error = nullptr;
  if (!FinishStatus(
          owl_fresh::owl_fresh_mojo_session_flush(_session, &ok, &c_error),
          c_error, error)) {
    return nil;
  }
  return @(ok);
...
- (nullable NSString*)profilePathWithError:(NSError**)error {
  ...
  char* c_error = nullptr;
  if (!FinishStatus(
          owl_fresh::owl_fresh_mojo_profile_get_path(_session, &path, &c_error),
          c_error, error)) {
    return nil;
  }
...
- (BOOL)navigateToURL:(NSString*)url error:(NSError**)error {
  char* c_error = nullptr;
  return FinishStatus(owl_fresh::owl_fresh_mojo_web_view_navigate(
                          _session, url.UTF8String, &c_error),
                      c_error, error);
}
...
- (BOOL)resizeWithWidth:(uint32_t)width
                 height:(uint32_t)height
                  ...
                  error:(NSError**)error {
  char* c_error = nullptr;
  return FinishStatus(owl_fresh::owl_fresh_mojo_web_view_resize(
                          _session, width, height, scale, &c_error),
                      c_error, error);
}
...
- (BOOL)setFocus:(BOOL)focused error:(NSError**)error {
  char* c_error = nullptr;
  return FinishStatus(
      owl_fresh::owl_fresh_mojo_web_view_set_focus(_session, focused, &c_error),
      c_error, error);
}
...
- (BOOL)sendKeyWithKeyDown:(BOOL)keyDown
  ...
  char* c_error = nullptr;
  return FinishStatus(
      owl_fresh::owl_fresh_mojo_input_send_key(
          _session, keyDown, keyCode, text.UTF8String, modifiers, &c_error),
      c_error, error);
}
...
- (nullable NSString*)surfaceTreeJSONWithError:(NSError**)error {
  char* result = nullptr;
  char* c_error = nullptr;
  if (!FinishStatus(owl_fresh::owl_fresh_mojo_surface_tree_get_json(
                        _session, &result, &c_error),
                    c_error, error)) {
    return nil;
  }
...
- (nullable NSNumber*)cancelActivePopupWithError:(NSError**)error {
  ...
  char* c_error = nullptr;
  if (!FinishStatus(
          owl_fresh::owl_fresh_mojo_native_surface_cancel_active_popup(
              _session, &ok, &c_error),
          c_error, error)) {
    return nil;
  }
...
- (nullable NSNumber*)cancelActiveFilePickerWithError:(NSError**)error {
  ...
  char* c_error = nullptr;
  if (!FinishStatus(
          owl_fresh::owl_fresh_mojo_native_surface_cancel_active_file_picker(
              _session, &ok, &c_error),
          c_error, error)) {
    return nil;
  }
...
- (nullable NSNumber*)openDevToolsWithMode:(uint32_t)mode
  ...
  BOOL ok = NO;
  char* c_error = nullptr;
  if (!FinishStatus(owl_fresh::owl_fresh_mojo_devtools_open(_session, mode, &ok,
                                                            &c_error),
                    c_error, error)) {
    return nil;
  }
...
- (nullable NSNumber*)closeDevToolsWithError:(NSError**)error {
  ...
  char* c_error = nullptr;
  if (!FinishStatus(
          owl_fresh::owl_fresh_mojo_devtools_close(_session, &ok, &c_error),
          c_error, error)) {
    return nil;
  }
  return @(ok);
  ...
  if (!session) {
    if (error) {
      *error = OwlFreshNSError(
          @"owl_fresh::owl_fresh_mojo_session_create returned null");
    }
    return nil;
  }
